...
import json
import re
from Standard_ACE.parserACE import Parser
...
def find_token_index(tokens, start_pos, end_pos, phrase):
    ...
    try:
        if len(check_start_match_list)>1:
            ...
            length = len(json.loads(nlp.annotate(phrase,properties={'annotators': 'tokenize'}))['tokens'])
...
def preprocessing(data_type, files):
    ...
    for file in tqdm(files):
        ...
        for item in parser.get_data():
            ...
            try:
                nlp_text = nlp.annotate(item['sentence'], properties={'annotators': 'tokenize'})
...
path = 'C:/Users/dell/Desktop/package/ace_2005_td_v7/data/English/'
